...
export default class PageLayout extends React.Component {
  render() {
    return (
      <React.Fragment>
        <div className={STYLES_NAVIGATION}>{this.props.navigation}</div>
        <div className={STYLES_HEADER}>{this.props.header}</div>
        <div className={STYLES_BODY}>
          {this.props.children}
          {this.props.sidebar ? (
            <div className={STYLES_SIDEBAR}>
              <div className={STYLES_TRUE_SIDEBAR}>
                <div className={STYLES_SIDEBAR_HEADER}>
                  <div
                    className={STYLES_BLOCK}
                    onClick={this.props.onDismissSidebar}
                  >
                    <SVG.Dismiss height="24px" />
                  </div>
                </div>
                <div className={STYLES_SIDEBAR_CONTENT}>
                  {this.props.sidebar}
                </div>
              </div>
            </div>
